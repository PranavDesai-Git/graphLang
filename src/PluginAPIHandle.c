...
static Node *resolveHandle(Handle h) {
    if (h < 0 || h >= rootCount) return NULL;
    return gcRoots[h];
}
...
static void api_registerNative(char *name, Handle (*func)(Handle args, Handle env)) {
    // We cast the handle function back to the internal raw Node pointer function type.
    // The Evaluator handles this gracefully when it unpacks the plugin return value.
...
static Handle api_createFunction(Handle funcExpr, Handle args) {
    return makeHandle(createFunction(resolveHandle(funcExpr), resolveHandle(args)));
}
...
static void api_pushRoot(Handle node) {
    pushRoot(resolveHandle(node));
}

static void api_popRoot(void) {
    popRoot();
}
...
static int api_getLiteral(Handle n) {
    return getLiteral(resolveHandle(n));
}

static char *api_getVarName(Handle n) {
    return getVarName(resolveHandle(n));
}

static char *api_getFuncName(Handle n) {
    return getFuncName(resolveHandle(n));
}

// Return the fully wrapped API Struct
VMAPI getHandleAPI(void) {
    VMAPI api = {
        .registerNative = api_registerNative,
